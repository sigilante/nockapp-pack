//! solitaire: a playable Klondike Solitaire served over HTTP, with ALL game logic
//! (deck shuffle/deal, legal-move checks, foundations, win detection) living in the
//! Hoon kernel. This binary just boots the kernel from `out.jam` and attaches the
//! library's HTTP driver.
//!
...
//! We default HTTP_PORT to 8087.
//!
//! ## Cache
//!
//! The driver caches GET responses keyed by URI. The board page is GET `/`, so a cached
//! board would go stale after a move. We set `EXPIRE_CACHE=1` (1-second TTL) so GET `/`
//! re-pokes the kernel at least every second (fresh board + a `metric: moves=<N>` slog
//! line). NOTE: `EXPIRE_CACHE=0` PANICS at this rev (`tokio::time::interval(Duration::ZERO)`
//! -> "period must be non-zero"), so we use 1, not 0. Moves themselves are POSTs, whose
//! responses are never cached and already carry the freshly re-rendered board. The big
//! `/style.css` sprite sheet is fetched once and cached by the browser (cache-control).
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    // nockd exports NOCKD_APP_PORT; honor it so the port is single-sourced in nockd.toml.
    let port = std::env::var("NOCKD_APP_PORT")
    ...
    tracing::info!("solitaire starting; HTTP port {port}");
    ...
    let mut nockapp: NockApp = boot::setup(&kernel, cli, &[], "solitaire", None)
        .await
    ...
    tokio::select! {
        ...
        _ = sigterm.recv() => {
            tracing::info!("solitaire: received SIGTERM; shutting down cleanly");
        }
        _ = tokio::signal::ctrl_c() => {
            tracing::info!("solitaire: received SIGINT; shutting down cleanly");
        }
    }
